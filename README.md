...
Setting `"useNoReadAhead": true` opens LMDB with `MDB_NORDAHEAD`, stopping the OS from prefetching neighbours that get evicted before use. Measured cold-cache A/B: **965.8 s vs 1,042.8 s — 7.4 % faster**, achieved while starting with 19 % *less* free RAM, with bit-identical FPR and filter size. It defaults to `false` because read-ahead is a genuine win when the database does fit in RAM. See `docs/performance.md` for the caveats (n = 1 per arm) and `TODO.md` for the remaining levers (higher queue depth, key-order compaction).
