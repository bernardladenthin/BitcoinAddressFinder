 ...
 * {@code numBlocks} and {@code k}; the kernel derives everything else from the key. The block index
 * is a multiply-shift ({@code mul_hi(hash, numBlocks)}), so the count need not be a power of two.
 *
 ...
 *
 * <p>Kernel metadata layout is {@code [numBlocks, k]}, deliberately shorter than the fuse filter's
 * five words: the blocked layout needs no segment geometry, since all probes of a key are confined
 * to one 512-bit block.
 *
 * @param words     the backing bit array, {@code numBlocks * 8} longs
 * @param numBlocks the number of 512-bit blocks
 * @param k         number of bits probed per key
 */
@SuppressWarnings("ArrayRecordComponent")
public record BlockedBloomGpuFilterData(long[] words, int numBlocks, int k) {

    /**
     * Renders the kernel metadata array in the layout {@code blockedbloom_contains} expects.
     *
     * @return {@code [numBlocks, k]}
     */
    public int[] toMetadata() {
        return new int[] {numBlocks, k};
    }
    ...
    public boolean equals(@Nullable Object o) {
        ...
        return numBlocks == other.numBlocks && k == other.k && Arrays.equals(words, other.words);
    }

    @Override
    public int hashCode() {
        int result = Objects.hash(numBlocks, k);
    ...
    public String toString() {
        ...
        return "BlockedBloomGpuFilterData[words.length=" + words.length + ", numBlocks=" + numBlocks + ", k=" + k
