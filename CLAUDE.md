...
- **Pluggable persistence backends + lookup benchmark — mostly DONE.** Originally a "research-and-plan placeholder" entry; the bulk of the 6-point plan has shipped on this branch. Status of each plan item:
  1. ~~**Either resurrect the in-memory HashSet/HashMap backend or remove the dead README/config references.**~~ **DONE for the implementation**: `persistence/inmemory/HashSetAddressPresence.java` is the resurrected HashSet snapshot (populated from LMDB once at startup, LMDB env then closed and GC'd). **DONE for the README**: `README.md` no longer references `loadToMemoryCacheOnInit`. **Still open — 4 stale example configs** (re-verified 2026-06-04; original list missed `config_AddressFilesToLMDB.json`): `examples/config_AddressFilesToLMDB.json`, `examples/config_Find_1OpenCLDevice.json`, `examples/config_Find_1OpenCLDeviceAnd2CPUProducer.json`, `examples/config_Find_SecretsFile.json` each still carry `"loadToMemoryCacheOnInit": false`. Jackson silently ignores the unknown key so it's not a runtime bug, but it actively misleads operators about what the JSON field does. Trivial cleanup: delete the line from all 4 files in one commit (operators who want the equivalent semantics should set `addressLookupBackend: HASHSET` instead).
  ...
  **Genuinely open work** (concise summary, sorted easiest first):
  - Remove `"loadToMemoryCacheOnInit": false` from the 4 stale example JSON files (5-min cleanup).
