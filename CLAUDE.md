...
- **Add a timeout to `AbstractProducer.waitTillProducerNotRunning`.** The spin-wait deliberately swallows `InterruptedException` (see `WAIT_TILL_NOT_RUNNING_RESTORES_INTERRUPT_FLAG`) so an external interrupt cannot abort shutdown mid-flight. But the loop has no other exit condition besides `state != RUNNING`, so a producer that hangs inside `produceKeys()` (stuck blocking I/O, infinite loop, OpenCL kernel that never returns) will hang `Finder.interrupt()` forever. Add a `Duration` parameter or a config-driven shutdown deadline that bounds the wait; on timeout, log loudly and move on so the rest of the shutdown chain still runs.
