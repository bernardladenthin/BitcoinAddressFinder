...
- **Make `-Werror` clean (BAF-specific).** `pom.xml` enables `-Xlint:all,-serial,-options,-classfile` (warnings visible) but stops short of `-Werror`. Items 1, 3, and 4 below have been cleared on the `claude/inspiring-carson-D831V` branch; the remaining work to flip the switch is items 2, 5, 6, plus the ~30 outstanding Error Prone warnings tracked separately:
