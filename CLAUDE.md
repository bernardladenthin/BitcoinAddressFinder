...
- **Further-strictness open points (cross-repo, not yet done).** Items below are tracked across all four Bernard-Ladenthin Java repos and can be picked up incrementally:
  ...
  - ~~**`--release N`** instead of `-source N -target N`~~ — **DONE for this repo**. Main compile uses `<release>21</release>` (`pom.xml:313`); `module-info-compile` execution stays at `--release 9`; `default-testCompile` overrides back to `<source>/<target>` because tests legitimately import `jdk.internal.ref.Cleaner` and `sun.nio.ch.DirectBuffer` (which `--release` forbids via `--add-exports`). The blocker was `ByteBufferUtility.java`'s direct `jdk.internal.misc.Unsafe` import; migrated to reflectively-resolved `sun.misc.Unsafe` (always-exported via `jdk.unsupported`) with a `@Nullable UNSAFE` field — the static initializer wraps the reflective lookup in try/catch so non-HotSpot JVMs (OpenJ9, GraalVM Native Image, Android) get `UNSAFE == null` rather than `ExceptionInInitializerError`; `freeByteBuffer` gained a null guard so it becomes a no-op on those platforms (JVM Cleaner handles the buffer naturally).
...
- **Pluggable persistence backends + lookup benchmark — mostly DONE.** Originally a "research-and-plan placeholder" entry; the bulk of the 6-point plan has shipped on this branch. Status of each plan item:
  ...
  6. ~~**Lookup benchmark** — partially done: `src/test/java/net/ladenthin/bitcoinaddressfinder/persistence/AddressLookupBenchmarkTest.java` times `containsAddress` across all 4 backends on the same dataset.~~ ✅ **MIGRATED to JMH**: `src/test/java/net/ladenthin/bitcoinaddressfinder/benchmark/AddressLookupBenchmark.java` is the JMH version (`@Param({"LMDB_ONLY","BLOOM","HASHSET","TRUNCATED_LONG_64"})`, `Mode.AverageTime`, `OutputTimeUnit.NANOSECONDS`, same `0xC0FFEE` seed + 2 048 LMDB entries + Bloom FPP 0.01 as the predecessor for apples-to-apples comparison). The old JUnit timing test was deleted — its commit body explicitly said "JUnit comparison benchmark (not JMH)" and the file's Javadoc admitted "without setting up a separate JMH harness", but the JMH harness already existed (created by the owner 2 days earlier in PR #215, c.f. `BitHelperBenchmark`/`ByteSwapBenchmark`/`PublicKeyHashBenchmark`). Run with `mvn test-compile exec:java -Dexec.args="AddressLookupBenchmark"` (or `-prof gc` for allocation profile). Does NOT run in `mvn test` — matches the existing JMH convention and the new BAF test-execution policy.
  ...
  **Genuinely open work** (concise summary, sorted easiest first):
  ...
  - ~~Migrate `AddressLookupBenchmarkTest` to the JMH `benchmark/` module (1–2 h).~~ ✅ **DONE** — see item 6 above (`AddressLookupBenchmark.java` is the JMH version; old JUnit timing test deleted).
