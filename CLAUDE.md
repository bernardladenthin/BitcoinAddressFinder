...
- **Pluggable persistence backends (LMDB / HashMap / BloomFilter) + lookup benchmark.** Owner recollection is that a HashMap-based persistence existed pre-Git; investigation confirms it does not exist anywhere in this repo's git history (boot commit `2c8e9f1` 2026-05-08; `grep -S"HashMap"` against `persistence/` is empty). Today the layer is:
  ...
  1. **Pull the BloomFilter accelerator out of `LMDBPersistence`** into its own concern (e.g. `persistence/bloom/BloomFilterAccelerator`) so it can wrap any backend, not just LMDB. Keep the existing `useBloomFilter` config working.
  2. **Add standalone backends in sibling sub-packages:**
     - `persistence/hashmap/HashMapPersistence` — in-memory `Map<ByteBuffer, Coin>` or `Map<byte[]wrap, Coin>`; pay attention to memory budget (realistic address sets are 10⁸-10⁹ entries; rough estimate 24-32 bytes per entry of overhead alone, 5-15 GB heap for the full set).
  ...
  3. **Make the backend selectable from config** under `consumerJava` (currently `lmdbConfigurationReadOnly` is mandatory). Likely shape: `consumerJava.persistence: { type: "LMDB"|"HASHMAP"|"BLOOM", ... }` with the existing LMDB config as one variant. Default stays LMDB for backward compat with existing example configs.
  4. **Optional: a layered/chained backend** so the operator can pick "Bloom in front of LMDB" (current behaviour, but explicit) or "HashMap in front of LMDB" (warm-cache mode for hot addresses) via composition of `Persistence` implementations.
  5. **JMH lookup benchmark** comparing `containsAddress` + `getAmount` throughput on identical datasets across the three backends (cold and warm). The repo already has a JMH benchmark module - extend it. Measure also the build / load time and steady-state RSS for each backend so the trade-off (memory vs. latency vs. setup cost) is visible.

  **Out of scope of this TODO:** actually executing any of the above. This entry is a research-and-plan placeholder so the architecture decision is on record.
